...
BM.EXERCISES = [
     ...
     ============================================================ */
  {
    ...
    barem: [
      { descriere: "Zerourile și semnul funcției $f$", puncte_maxime: 2, explicatie: "Pentru a determina zerourile funcției, rezolvăm ecuația $f(x) = 0$, adică $x^4 - 4x^3 = 0$. Prin factorizare obținem $x^3(x - 4) = 0$, de unde rezultă rădăcinile $x = 0$ și $x = 4$. Deoarece integrăm pe intervalul $[-1, 1]$, analizăm semnul funcției pe subintervalele delimitate de rădăcina $x = 0$. Pe intervalul $[-1, 0]$ avem $x^3 \\le 0$ și $x - 4 < 0$, deci $f(x) \\ge 0$, iar pe intervalul $[0, 1]$ avem $x^3 \\ge 0$ și $x - 4 < 0$, deci $f(x) \\le 0$." },
      { descriere: "$\\int_{-1}^{1} |f(x)|\\, dx = \\int_{-1}^{0} (x^4 - 4x^3)\\, dx + \\int_{0}^{1} (4x^3 - x^4)\\, dx$", puncte_maxime: 2, explicatie: "Utilizăm proprietatea de aditivitate a integralei definite în raport cu intervalul de integrare și explicităm modulul în funcție de semnul stabilit anterior. Deoarece $f(x) \\ge 0$ pe $[-1, 0]$, avem $|f(x)| = f(x) = x^4 - 4x^3$. Pe intervalul $[0, 1]$, unde $f(x) \\le 0$, avem $|f(x)| = -f(x) = 4x^3 - x^4$. Astfel, scriem integrala inițială ca sumă de două integrale:\n$$\\int_{-1}^{1} |f(x)|\\, dx = \\int_{-1}^{0} (x^4 - 4x^3)\\, dx + \\int_{0}^{1} (4x^3 - x^4)\\, dx$$" },
      { descriere: "Determinarea unei primitive a funcției $f$", puncte_maxime: 2, explicatie: "Pentru a calcula integralele obținute, determinăm o primitivă $F(x)$ a funcției $f(x) = x^4 - 4x^3$. Folosim formulele de integrare pentru funcții putere, $\\int x^n\\, dx = \\frac{x^{n+1}}{n+1}$ și liniaritatea integralei. Calculăm primitiva astfel:\n$$F(x) = \\int (x^4 - 4x^3)\\, dx = \\frac{x^5}{5} - 4 \\cdot \\frac{x^4}{4} = \\frac{x^5}{5} - x^4$$" },
      { descriere: "Aplicarea formulei Newton-Leibniz și obținerea valorii integralei, egală cu $\\boxed{2}$", puncte_maxime: 2, explicatie: "Aplicăm formula Newton-Leibniz pentru fiecare dintre cele două integrale, utilizând primitiva determinată anterior. Pentru prima integrală obținem:\n$$\\int_{-1}^{0} (x^4 - 4x^3)\\, dx = F(0) - F(-1) = 0 - \\left(-\\frac{1}{5} - 1\\right) = \\frac{6}{5}$$\nPentru a doua integrală obținem:\n$$\\int_{0}^{1} (4x^3 - x^4)\\, dx = -\\left(F(1) - F(0)\\right) = -\\left(\\frac{1}{5} - 1\\right) = \\frac{4}{5}$$\nAdunând cele două rezultate, obținem valoarea finală a integralei:\n$$\\frac{6}{5} + \\frac{4}{5} = 2$$" }
    ]
  },
  {
    ...
    barem: [
      { descriere: "$t = \\sqrt{x+1}$, $x = t^2 - 1$, $dx = 2t\\, dt$", puncte_maxime: 3, explicatie: "Mai întâi, simplificăm expresia de sub integrală înlocuind $f(x) = \\frac{x^2}{x-2}$, ceea ce ne dă $\\frac{f(x)(x-2)}{x\\sqrt{x+1}} = \\frac{x^2}{x\\sqrt{x+1}} = \\frac{x}{\\sqrt{x+1}}$. Pentru a calcula integrala, utilizăm metoda substituției și notăm $t = \\sqrt{x+1}$. Prin ridicare la pătrat obținem $t^2 = x + 1$, de unde rezultă $x = t^2 - 1$, iar prin diferențiere obținem $dx = 2t\\, dt$." },
      { descriere: "Obținerea $2\\int_{2}^{3} (t^2 - 1)\\, dt$", puncte_maxime: 1, explicatie: "Schimbăm limitele de integrare pentru noua variabilă $t$: pentru limita inferioară $x = 3$ obținem $t = \\sqrt{3+1} = 2$, iar pentru limita superioară $x = 8$ obținem $t = \\sqrt{8+1} = 3$. Înlocuind expresiile determinate anterior în integrala inițială, obținem: $$\\int_{3}^{8} \\frac{x}{\\sqrt{x+1}}\\, dx = \\int_{2}^{3} \\frac{t^2-1}{t} \\cdot 2t\\, dt$$ Prin simplificarea termenului $t$ de la numitor cu cel de la numărător și scoaterea constantei $2$ în fața integralei, ajungem la forma finală: $$2\\int_{2}^{3} (t^2 - 1)\\, dt$$" },
      { descriere: "Determinarea unei primitive a funcției de sub simbolul integralei", puncte_maxime: 2, explicatie: "Pentru a determina o primitivă a funcției de sub integrală, integrăm termen cu termen expresia $t^2 - 1$ folosind formulele fundamentale de integrare. Aplicăm formula $\\int t^n\\, dt = \\frac{t^{n+1}}{n+1}$ pentru $t^2$ și obținem $\\frac{t^3}{3}$, iar pentru constanta $1$ obținem $t$. Astfel, o primitivă a funcției $g(t) = t^2 - 1$ este: $$G(t) = \\frac{t^3}{3} - t$$" },
      { descriere: "Aplicarea formulei Newton-Leibniz și obținerea valorii integralei, egală cu $\\boxed{\\frac{32}{3}}$", puncte_maxime: 2, explicatie: "Aplicăm formula Newton-Leibniz pentru primitiva găsită, înmulțită cu constanta $2$ din fața integralei, calculând $2\\left(G(3) - G(2)\\right)$. Evaluăm primitiva în limitele de integrare: $G(3) = \\frac{3^3}{3} - 3 = 9 - 3 = 6$ și $G(2) = \\frac{2^3}{3} - 2 = \\frac{8}{3} - 2 = \\frac{2}{3}$. Efectuăm calculul final și obținem valoarea integralei: $$2\\left(6 - \\frac{2}{3}\\right) = 2 \\cdot \\frac{16}{3} = \\frac{32}{3}$$" }
    ]
  },
